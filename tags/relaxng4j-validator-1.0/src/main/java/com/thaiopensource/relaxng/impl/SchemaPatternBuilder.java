...
public class SchemaPatternBuilder extends PatternBuilder {
  ...
  Pattern makeData(com.relaxng4j.datatype.Datatype dt) {
    noteDatatype(dt);
    Pattern p = new DataPattern(dt);
    return schemaInterner.intern(p);
  }

  Pattern makeDataExcept(com.relaxng4j.datatype.Datatype dt, Pattern except, Locator loc) {
    noteDatatype(dt);
  ...
  private void noteDatatype(com.relaxng4j.datatype.Datatype dt) {
